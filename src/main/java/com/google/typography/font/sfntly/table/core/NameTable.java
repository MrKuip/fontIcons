    ...
    implements Iterable<NameTable.NameEntry>
{
  ...
  {
    return this.data.readUShort(Offset.nameRecordPlatformId.offset + this.offsetForNameRecord(index));
  }
  ...
  {
    return this.data.readUShort(Offset.nameRecordEncodingId.offset + this.offsetForNameRecord(index));
  }
  ...
  {
    return this.data.readUShort(Offset.nameRecordLanguageId.offset + this.offsetForNameRecord(index));
  }
  ...
  {
    return this.data.readUShort(Offset.nameRecordNameId.offset + this.offsetForNameRecord(index));
  }
  ...
  {
    return this.data.readUShort(Offset.nameRecordStringLength.offset + this.offsetForNameRecord(index));
  }
  ...
  {
    return this.data.readUShort(Offset.nameRecordStringOffset.offset + this.offsetForNameRecord(index))
        + this.stringOffset();
  }
   ...
   * requested name then <code>null</code> is returned.
   * 
  ...
  /**
   * Get the name as a String for the given name record. If there is no encoding
   * conversion available for the name record then a best attempt String will be
   * returned.
   * 
   * @param index the index of the name record
   * @return the name
  ...
  /**
   * Get the name as a String for the specified name. If there is no entry for the
   * requested name then <code>null</code> is returned. If there is no encoding
   * conversion available for the name then a best attempt String will be
   * returned.
   * 
   * @param platformId the platform id
   * @param encodingId the encoding id
  ...
  {
    return new NameEntry(this.platformId(index), this.encodingId(index), this.languageId(index), this.nameId(index),
        this.nameAsBytes(index));
  }

  /**
   * Get the name entry record for the specified name. If there is no entry for
   * the requested name then <code>null</code> is returned.
   * 
   * @param platformId the platform id
   * @param encodingId the encoding id
   * @param languageId the language id
   ...
   * @return the name entry
   */
  public NameEntry nameEntry(final int platformId, final int encodingId, final int languageId, final int nameId)
  {
      ...
      implements Comparable<NameEntryId>
  {
    ...
    {
      /*
       * - this takes advantage of the sizes of the various entries and the fact that
       * the ranges of their values have an almost zero probability of ever changing -
       * this allows us to generate a unique hash at low cost - if the ranges do
       * change then we will potentially generate non-unique hash values which is a
       * common result
       */
      return ((this.encodingId & 0x3f) << 26) | ((this.nameId & 0x3f) << 16) | ((this.platformId & 0x0f) << 12)
          | (this.languageId & 0xff);
    }

    /**
     * Name entries are sorted by platform id, encoding id, language id, and name id
     * in order of decreasing importance.
     *
     * @return less than zero if this entry is less than the other; greater than
     *         zero if this entry is greater than the other; and zero if they are
     *         equal
     *
  ...
  public static class NameEntry
  {
    ...
    protected NameEntry(int platformId, int encodingId, int languageId, int nameId, byte[] nameBytes)
    {
    ...
    /**
     * Get the name as a String. If there is no encoding conversion available for
     * the name bytes then a best attempt String will be returned.
     * 
     * @return the name
     */
    public String name()
      ...
      implements Iterator<NameEntry>
  {
    ...
    public boolean hasNext()
    {
      ...
      {
        if (filter.accept(platformId(this.nameIndex), encodingId(this.nameIndex), languageId(this.nameIndex),
  ...
  // TODO(stuartg):
  // do the conversion by hand to detect conversion failures (i.e. no character in
  // the encoding)
    ...
    extends SubTableContainerTable.Builder<NameTable>
  {
    ...
    protected int subDataSizeToSerialize()
    {
      ...
      int size = NameTable.Offset.nameRecordStart.offset
          + this.nameEntryMap.size() * NameTable.Offset.nameRecordSize.offset;
    ...
    {
      int stringTableStartOffset = NameTable.Offset.nameRecordStart.offset
          + this.nameEntryMap.size() * NameTable.Offset.nameRecordSize.offset;
      ...
      for (Map.Entry<NameEntryId, NameEntryBuilder> entry : this.nameEntryMap.entrySet())
      {
        ...
        newData.writeUShort(nameRecordOffset + NameTable.Offset.nameRecordNameId.offset, entry.getKey().getNameId());
        newData.writeUShort(nameRecordOffset + NameTable.Offset.nameRecordStringLength.offset,
            entry.getValue().nameAsBytes().length);
        newData.writeUShort(nameRecordOffset + NameTable.Offset.nameRecordStringOffset.offset, stringOffset);
        nameRecordOffset += NameTable.Offset.nameRecordSize.offset;
        // string table
        byte[] nameBytes = entry.getValue().nameAsBytes();
        if (nameBytes.length > 0)
        {
          stringOffset += newData.writeBytes(stringOffset + stringTableStartOffset, entry.getValue().nameAsBytes());
        }
      }
