  ...
  extends Table
{
  ...
  // method can therefore throw various runtime exceptions if the table is
  // corrupted.
  /**
   * Get a list containing the names in the table. Since parsing this list is
   * potentially expensive and may throw an exception when data is corrupted,
   * parsing is deferred until first use.
   * 
   * Also note that the return value is only valid for version 2 tables
   * (potentially to be expanded to other versions later). A non-null value is
   * guaranteed for version 2 (only).
   */
  ...
  private List<String> parse()
  {
    ...
    if (version() == VERSION_2)
    {
      ...
      while (index < dataLength() && names.size() != numberOfGlyphs())
      {
